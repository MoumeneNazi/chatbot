...
class MentalHealthChatbot:
    ...
    def _generate_response_gemini(self, prompt):
        """Generate a response using Google Vertex AI API"""
        try:
            if not self.gemini_available:
                return None
                
            logger.info(f"Calling Vertex AI API with prompt: {prompt[:50]}...")
            ...
            logger.info(f"Vertex AI response received ({len(content)} chars)")
            return content
        except Exception as e:
