...
async def promote_to_therapist(
    ...
    try:
        ...
        if user.role == "therapist":
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"User {username} is already a therapist"
            )
            
        # Promote user
        promoted_user = promote_user(username, db)
        if not promoted_user:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to promote user {username}"
            )
            
        logger.info(f"Successfully promoted user {username} to therapist")
